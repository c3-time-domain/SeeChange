...
def single_sextrsky( imagedata, maskdata=None, sigcut=3 ):
    ...
    while not done:
        ...
        SCLogger.get().debug( f'single_sextrsky: med={med:.2f}, mean={mean:.2f}, sdev={sdev:.2f}, n={w.sum()}' )
        if w.sum() > lastn:
            SCLogger.get().warning( "single_sextrsky: n increased" )
        if w.sum() == lastn:
            done = True
        lastn = w.sum()

    if math.fabs( mean - med ) / mean > 0.3:
        SCLogger.get().debug( f'mean={mean}, med={med}, using just median for sky estimate' )
        sky = med
    else:
...
def main():
    ...
    if args.boxwid == 0:        
        ...
        sky = np.median( skyim )
    SCLogger.get().debug( f'Sky: {sky}; σ: {sig}' )

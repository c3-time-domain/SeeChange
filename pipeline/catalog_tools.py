...
def download_gaia_dr3( minra, maxra, mindec, maxdec, padding=0.1, minmag=18., maxmag=22. ):
    ...
    SCLogger.get().info( f'Querying NOIRLab Astro Data Archive for Gaia DR3 stars' )
    ...
    SCLogger.get().debug( f'gaia_query is "{gaia_query}"' )
    ...
    for i in range(5):
        try:
            ...
            SCLogger.get().info( f"Failed Gaia download: {str(e)}" )
            if i < 4:
                SCLogger.get().info( "Sleeping 5s and retrying gaia query after failed attempt." )
                time.sleep(5)
    else:
        errstr = f"Gaia query failed after {i} repeated failures."
        SCLogger.get().error( errstr )
    ...
    SCLogger.get().debug( f"Writing {len(fitstab)} gaia stars to {ofpath}" )
        ...
        for maxmag in maxmags:
            ...
            if q.count() > 0:
                ...
                if not os.path.isfile( catexp.get_fullpath() ):
                    SCLogger.get().info( f"CatalogExcerpt {catexp.id} has no file at {catexp.filepath}")

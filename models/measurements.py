...
class Measurements(Base, UUIDMixin, SpatiallyIndexed, HasBitFlagBadness):
    ...
    x = sa.Column(
        sa.Integer,
        nullable=False,
        doc="x pixel coordinate on the sub image"
    )

    y = sa.Column(
        sa.Integer,
        nullable=False,
        doc="y pixel coordinate on the sub image"
    )
    ...
    width = sa.Column(
        ...
        nullable=False,
        index=True,
        doc="Width (1σ for a Gaussian) of the source in the cutout. "
    ...
    # Removing this -- this doesn't make sense as a method of a Measurements object, which
    #   is already at a specific point.

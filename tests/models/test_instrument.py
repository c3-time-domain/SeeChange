...
def test_decam_download_and_load_exposure( code_version, decam_origin_exposures ):
    eids = []
    try:
        with SmartSession() as session:
            # Deliberately choosing different indices here than in test_decam_download_orign_exposure
            # so that we'll make sure the download works when called from download_and_load_exposures
            expdexes = [ 1, 2 ]
            exposures = decam_origin_exposures.download_and_load_exposures( indexes=expdexes, clobber=False,

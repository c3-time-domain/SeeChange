...
def test_main_page( browser, conductor_url ):
    ...
    el = WebDriverWait( browser, timeout=10 ).until( lambda d: d.find_element(By.TAG_NAME, 'h1' ) )
...
def test_conductor_not_logged_in( conductor_url ):
    res = requests.post( f"{conductor_url}/status", json={ "command": "status" } )
    assert res.status_code == 500
    assert res.text == "Not logged in"


def test_conductor_uninitialized( conductor_url ):
    res = requests.post( f"{conductor_url}/status", json={ "command": "status" } )

        ...
        with SmartSession(session) as session:
            ...
            if ds.image is None and cache_dir is not None and cache_base_name is not None:
                ...
                if os.path.isfile(cache_path):
                    SCLogger.get().debug('loading image from cache. ')
            ...
            if ds.image is None:  # make the preprocessed image
                SCLogger.get().debug('making preprocessed image. ')
                ...
                if cache_dir is not None and cache_base_name is not None and output_path != cache_path:
                    ...
                    SCLogger.get().debug(f'Saving image to cache at: {output_path}')
            ...
            if cache_dir is not None and cache_base_name is not None:
                ...
                cache_path = os.path.join(cache_dir, cache_name)
                if os.path.isfile(cache_path):
                    SCLogger.get().debug('loading source list from cache. ')
                ...
                cache_path = os.path.join(cache_dir, cache_name)
                if os.path.isfile(cache_path):
                    SCLogger.get().debug('loading PSF from cache. ')
            ...
            if ds.sources is None or ds.psf is None:  # make the source list from the regular image
                SCLogger.get().debug('extracting sources. ')
                ds = extractor.run(ds)
                ds.sources.save()
            ...
            if cache_dir is not None and cache_base_name is not None:
                ...
                cache_path = os.path.join(cache_dir, cache_name)
                if os.path.isfile(cache_path):
                    SCLogger.get().debug('loading WCS from cache. ')
            ...
            if ds.wcs is None:  # make the WCS
                SCLogger.get().debug('Running astrometric calibration')
            ...
            if cache_dir is not None and cache_base_name is not None:
                ...
                cache_path = os.path.join(cache_dir, cache_name)
                if os.path.isfile(cache_path):
                    SCLogger.get().debug('loading zero point from cache. ')
            ...
            if ds.zp is None:  # make the zero point
                SCLogger.get().debug('Running photometric calibration')
            ...
            if cache_dir is not None:
                ...
                if os.path.isfile(os.path.join(cache_dir, cache_name)):
                    SCLogger.get().debug('loading subtraction image from cache. ')
            ...
            if os.path.isfile(cache_name):
                SCLogger.get().debug('loading detections from cache. ')
            ...
            if os.path.isfile(cache_name):
                SCLogger.get().debug('loading cutouts from cache. ')
            ...
            if os.path.isfile(cache_name):  # note that the cache contains ALL the measurements, not only the good ones
                SCLogger.get().debug('loading measurements from cache. ')

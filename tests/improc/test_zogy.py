        ...
        for new_seeing in seeing_values:
            for ref_bkg in background_values:
                for new_bkg in background_values:
                    # SCLogger.get().debug(f'seeing: {ref_seeing}, {new_seeing} | bkg: {ref_bkg}, {new_bkg}')
                    ...
                    for i, f in enumerate(fluxes):
                        ...
                        # SCLogger.get().debug((expected - measured) / expected)
...
def test_subtraction_jitter_noise():
    ...
    for jitter in jitter_values:
        # SCLogger.get().debug(f'jitter: {jitter}')
